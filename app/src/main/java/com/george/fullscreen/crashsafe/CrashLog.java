...
public class CrashLog {
    public static final String TAG = "CrashLog-测试";
    ...
    /**
     * Android版本号(release,7.0)
     * app的versionName，versionCode
     * Build:反射出系统属性（27个，cpu架构，厂商信息等）
     * @param ctx
     * @return map集合：设备和app信息
     */
    private static Map<String, String> collectDeviceInfo(Context ctx) {
        Map<String, String> infos = new TreeMap<>();
        try {
            infos.put("systemVersion", Build.VERSION.RELEASE);// 7.0
            PackageManager pm = ctx.getPackageManager();
            PackageInfo pi = pm.getPackageInfo(ctx.getPackageName(), PackageManager.GET_ACTIVITIES);
            if (pi != null) {
                String versionName = pi.versionName == null ? "null" : pi.versionName;
                String versionCode = pi.versionCode + "";//  pi.getLongVersionCode();// 需要try下 NoSuchMethodError，因为在api24不存在这个方法。
        ...
        Field[] fields = Build.class.getDeclaredFields();// 系统属性
    ...
    private static void saveCrashInfo2File(Context context, Throwable ex, Map<String, String> infos) {
        ...
        for (Map.Entry<String, String> entry : infos.entrySet()) {// map-->字符串
