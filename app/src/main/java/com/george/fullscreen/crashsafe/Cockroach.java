...
public final class Cockroach {
    private static final String TAG = "Cockroach-测试";
...
//        }
        ...
        initActivityKiller();
        //所有线程异常拦截，由于主线程的异常都被我们catch住了，所以下面的代码拦截到的都是子线程的异常。如果通过hook拦截过了异常，则这里不会被监听到。
        Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
            @Override
            public void uncaughtException(Thread t, Throwable e) {// 监听：不在activity生命周期Ing中的crash（当前界面已经显示，通过点击事件导致的null异常则会在这里抛出）
...
//        }
    ...
    /**
     * 在activity消息中发生的异常监听，替换ActivityThread.mH.mCallback实现
     * @throws Exception
     */
...
//                }
    ...
    private static void safeMode() {
        sIsSafeMode = true;
        if (sExceptionHandler != null) {// 标记当前已经crash过了
    ...
    /**
     * view measure layout draw时抛出异常会导致Choreographer挂掉(eg:testView.setText("")中testView为null)
     * <p>
    ...
    private static void isChoreographerException(Throwable e) {
        ...
        for (int i = elements.length - 1; i > -1; i--) {
            if (elements.length - i > 20) {//只要前20条信息。其他的是抛弃
                return;
            }
