...
public class CrashLogFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        decoration.setDrawable(getResources().getDrawable(R.drawable.list_divider_horizontal));// 间距
    ...
    private void readFileList() {
        fileReadHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                List<File> fs = Arrays.asList(file.listFiles());// 历史：多个crash文件
                Collections.sort(fs, new Comparator<File>() {// 排序：从新-》旧
                    @Override
                ...
                for (File f : fs) {
                    logs.add(new Log(f, f.getName(), null));// 映射成实体log
                }
                setFileList(logs);
            }
        });
    ...
    class LogAdapter extends RecyclerView.Adapter<LogAdapter.LogVH> {
        ...
        public void onBindViewHolder(@NonNull LogVH holder, int position) {
            ...
            if (log.content == null) {// 仅显示crash文件名
                holder.content.setVisibility(View.GONE);
                holder.copy.setVisibility(View.INVISIBLE);
            } else {// crash文件名+crash信息
        ...
        class LogVH extends RecyclerView.ViewHolder {
            ...
            public LogVH() {
                ...
                title.setOnClickListener(new View.OnClickListener() {// 点击查看具体log
                    @Override
